...
def create_question_hash(train_df, test_df):
    ...
    train_qs = np.dstack([train_df["question1"], train_df["question2"]]).flatten() #equals to simply append q2 list after q1 list
    test_qs = np.dstack([test_df["question1"], test_df["question2"]]).flatten()
    all_qs = np.append(train_qs, test_qs)
    all_qs = pd.DataFrame(all_qs)[0].drop_duplicates() #Return DataFrame with duplicate rows removed, optionally only considering certain columns
    all_qs.reset_index(inplace=True, drop=True)
    #https://pandas.pydata.org/pandas-docs/stable/generated/pandas.Series.to_dict.html
